...
import { Rasa, Rasy, dataHrdinu } from "./data.js";
...
abstract class Hrdina {
    ...
    constructor(
        id: number, jmeno: string, level: number, zakladniHP: number,
        sila: number, obratnost: number, inteligence: number, vydrz: number, rasa: Rasa
    ){
        if(id <= 0) { throw new Error("ID musí být kladné číslo"); }
        if(jmeno.trim() === "") { throw new Error("Jméno nesmí být prázdné"); }
        if(level < 1) { throw new Error("Level musí být alespoň 1"); }
        if(zakladniHP <= 0) { throw new Error("Základní HP musí být kladné číslo"); }
        if(!rasa) { throw new Error("Rasa musí být definována"); }
    ...
    public getJmeno(): string { return this.jmeno; }
    public getLevel(): number { return this.level; }
    public getZakladniHP(): number { return this.zakladniHP; }
    public getRasa(): string { return this.rasa.nazev; }
    ...
    public getVydrz(): number { return this.vydrz; }

    // Abstraktní metody, které musí každé povolání povinně implementovat po svém
    ...
    public getIkona(): string {
        if (this.getPovolani() === "Válečník") return "🛡️";
        if (this.getPovolani() === "Mág") return "🔮";
        return "🏹";
    }
...
class Valecnik extends Hrdina {
    constructor(id: number, jmeno: string, level: number, sila: number, obratnost: number, inteligence: number, vydrz: number, rasa: Rasa) {
    ...
    public getPovolani(): string { return "Válečník"; }
}
...
class Mag extends Hrdina {
    ...
    public getPovolani(): string { return "Mág"; }
}
...
class Lukostrelec extends Hrdina {
    ...
    public getPovolani(): string { return "Lukostřelec"; }
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("btn-jit-vytvorit")!.addEventListener("click", () => {
        ...
        (document.getElementById("editace-id") as HTMLInputElement).value = ""; 
        vynulujFormular();
        preklikniSekce(true); 
...
function preklikniSekce(doFormulare: boolean): void {
    if (doFormulare) {
...
function aktualizujSFPasPovolani(): void {
    ...
    pozice.forEach((povolaniIndex, slotId) => {
        ...
        const novyBtn = btn.cloneNode(true) as HTMLButtonElement;
...
function stylesCleanUp(str: string): string {
    return str.trim().replace(/\s+/g, ' ');
}

// Resetování stavu formuláře do výchozích hodnot
function vynulujFormular(): void {
    volneBody = 20;
    rucneRozdeleneBody = { sila: 0, obratnost: 0, inteligence: 0, vydrz: 0 };
    vybranaRasa = Rasy[0];
    indexPovolani = 0;
    (document.getElementById("vstup-jmeno") as HTMLInputElement).value = "";
    (document.getElementById("vstup-level") as HTMLInputElement).value = "1";
    generujRasyUI();
    obnovCislaBoduUI();
    aktualizujSFPasPovolani();
}
...
function generujRasyUI(): void {
    ...
    Rasy.forEach(rasa => {
        ...
        const f = (num: number) => num >= 0 ? `+${num}` : num.toString();
...
function generujAtributyUI(): void {
    const seznam = document.getElementById("atributy-ovladani")!;
    seznam.innerHTML = "";
    const popisky: Record<string, string> = { sila: "💪 Síla (Válečník)", obratnost: "🏹 Obratnost (Lukostřelec)", inteligence: "🔮 Inteligence (Mág)", vydrz: "❤️ Výdrž (Životy)" };
    ...
    ["sila", "obratnost", "inteligence", "vydrz"].forEach(klic => {
        ...
        document.getElementById(`btn-minus-${klic}`)!.addEventListener("click", () => upravAtribut(klic as any, -1));
        document.getElementById(`btn-plus-${klic}`)!.addEventListener("click", () => upravAtribut(klic as any, 1));
    });
}

// Změna stavu bodů v paměti (přičtení / odečtení volného bodu)
function upravAtribut(klic: "sila" | "obratnost" | "inteligence" | "vydrz", zmena: number): void {
...
function obnovCislaBoduUI(): void {
    document.getElementById("volne-body")!.textContent = volneBody.toString();
    
    const mapovaniBonusu: Record<string, number> = {
    ...
    for (let klic in rucneRozdeleneBody) {
        const rucnePridano = rucneRozdeleneBody[klic as "sila" | "obratnost" | "inteligence" | "vydrz"];
        ...
        (document.getElementById(`btn-minus-${klic}`) as HTMLButtonElement).disabled = rucnePridano <= 0;
        (document.getElementById(`btn-plus-${klic}`) as HTMLButtonElement).disabled = volneBody <= 0;
    }
}

// Načtení dat stávajícího hrdiny zpět do formuláře za účelem jeho úpravy
function otevriEditaciHrdiny(id: number): void {
    const hrdina = parta.find(h => h.getId() === id);
    if (!hrdina) return;

    document.getElementById("nadpis-formularu")!.textContent = `Úprava hrdiny: ${hrdina.getJmeno()}`;
    (document.getElementById("editace-id") as HTMLInputElement).value = id.toString();

    (document.getElementById("vstup-jmeno") as HTMLInputElement).value = hrdina.getJmeno();
    (document.getElementById("vstup-level") as HTMLInputElement).value = hrdina.getLevel().toString();
...
function smazHrdinu(id: number): void {
...
function zpracujFormularHrdiny(): void {
    const jmeno = (document.getElementById("vstup-jmeno") as HTMLInputElement).value.trim();
    const level = parseInt((document.getElementById("vstup-level") as HTMLInputElement).value) || 0;
    const editIdStr = (document.getElementById("editace-id") as HTMLInputElement).value;
    
    // Validace kompletnosti formuláře před uložením
    if (!jmeno) return alert("⚠️ Jméno nesmí být prázdné!");
    if (level < 1) return alert("⚠️ Level musí být alespoň 1!");
    if (volneBody > 0) return alert(`⚠️ Musíš rozdělit ještě ${volneBody} zbývajících bodů!`);
...
function obnovWebovouDruzinu(): void {
